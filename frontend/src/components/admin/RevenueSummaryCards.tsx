...
import { DollarSign, TrendingUp, Users, AlertCircle, Clock, RefreshCw } from 'lucide-react';
...
export function RevenueSummaryCards() {
  ...
  return (
    <>
      {dataUpdatedAt > 0 && (
        <div className="flex justify-end items-center gap-2 text-xs text-slate-500 mb-2 mt-[-10px]">
          <span className="flex items-center gap-1 px-3 py-1 bg-slate-800/50 rounded-full border border-slate-700/50">
            <Clock className="w-3.5 h-3.5" />
            Last checked: {new Date(dataUpdatedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}
          </span>
          <button
            onClick={() => refetch()}
            className="p-1.5 text-slate-500 hover:text-white transition-colors rounded-full"
